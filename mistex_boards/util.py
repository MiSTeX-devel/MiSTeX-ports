...
def add_designfiles(platform, coredir, mistex_yaml, toolchain, build_dir=None):
    ...
    verilog_includes = mistex_yaml[toolchain].get('verilog-include-paths', [])
    for command in verilog_includes:
        platform.add_verilog_include_path(replace_coredir(command))
    ...
    for source in mistex_yaml.get('sourcefiles', []):
        ...
        add_source(toolchain, platform, sourcepath, coredir, build_dir, use_template_sys)
...
def add_sources(toolchain, platform, coredir, build_dir, subdir, excludes, use_template_sys):
    ...
    for fname in os.listdir(sourcedir):
        ...
        if not (fname.endswith(".sv") or
                ...
                fname.endswith(".mif") or
                fname.endswith(".vhd")) or excluded:
            if excluded: print(f"{Fore.RED}    {fpath} is excluded...{Style.RESET_ALL}")
            continue

        add_source(toolchain, platform, fpath, coredir, build_dir, use_template_sys)

def add_source(toolchain, platform, fpath, coredir, build_dir, use_template_sys):
