...
def get_toolchain():
    ...
    if toolchain is None:
        toolchain = "vivado"
    return (toolchain, gui)

def run_gui(build_name, build_dir):
    lines  = []
    result = []
    cwd = os.getcwd()
...
def build_xilinx(platform_factory, coredir, core, fpga_name):
    mistex_yaml = yaml.load(open(join(coredir, "MiSTeX.yaml"), 'r'), Loader=yaml.FullLoader)

    toolchain, gui = get_toolchain()

    platform = platform_factory(toolchain)
    ...
    if toolchain.startswith("vivado"):
    ...
    soc = BaseSoC(platform, core_name=core, fpga_name=fpga_name, toolchain=toolchain)
    builder = Builder(soc,
        build_backend="litex",
    ...
    if gui: run_gui(build_name, build_dir)
